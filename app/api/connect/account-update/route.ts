  ...
  try {
    ...
    if (needsUpdate) {
      ...
      const updateLink = await stripe.accountLinks.create({
        account: accountId,
        type: 'account_update',
        refresh_url: `${process.env.NEXTAUTH_URL || 'https://monniegift.nl'}/stripe/refresh`,
        return_url: `${process.env.NEXTAUTH_URL || 'https://monniegift.nl'}/stripe/terug?account_id=${accountId}`,
      });
