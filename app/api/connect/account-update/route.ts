  ...
  try {
    ...
    if (needsUpdate) {
      ...
      const updateLink = await stripe.accountLinks.create({
        account: accountId,
        type: 'account_update',
        refresh_url: `${process.env.NEXTAUTH_URL || 'https://monnie-gift-v222.vercel.app'}/stripe/refresh`,
        return_url: `${process.env.NEXTAUTH_URL || 'https://monnie-gift-v222.vercel.app'}/stripe/terug?account_id=${accountId}`,
      });
